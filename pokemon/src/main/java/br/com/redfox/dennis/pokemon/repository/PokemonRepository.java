...
public interface PokemonRepository extends PagingAndSortingRepository<Pokemon, Long> {
	...
	@Query("SELECT pokemon FROM Pokemon pokemon" + " INNER JOIN  pokemon.tipe tipe "
			+ "INNER JOIN pokemon.weather weather where ( :id is NULL or pokemon.id = :id) AND "
			...
			+ "( :weather is NULL or pokemon.weather = :weather)")
